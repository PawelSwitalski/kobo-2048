...
`-DKOBO_2048_BACKEND=`: `none` (core + tests only), `sdl` (desktop simulator)
...
build/sim/Release/kobo_2048 --width 1264 --height 1680 --dpi 300
```
...
cmake -B build/kobo -DCMAKE_TOOLCHAIN_FILE=cmake/kobo-toolchain.cmake \
      -DKOBO_2048_BACKEND=fbink
cmake --build build/kobo
tools/package.sh build/kobo                # -> dist/kobo_2048.zip
```

`dist/kobo_2048.zip` is the install package described in
...
tests first, then the Kobo cross-build, uploading `kobo_2048.zip` as a
workflow artifact.
...
```
src/core/       Pure app/game logic, no OS calls (board, game session, best score, display settings)
...
src/ui/         Screens (game, settings), widgets, theme
