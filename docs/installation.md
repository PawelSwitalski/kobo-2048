...
| Launcher | Notes |
|---|---|
| **NickelMenu** (recommended) — a **Kobo 2048** entry in the **More** menu | Firmware 4.x only |
...
1. Build `dist/kobo_2048.zip` (see [building.md](building.md)), or download
...
5. Open **More** (bottom navigation) — a **Kobo 2048** entry appears
...
(`.adds/kfmon/config/kobo_2048.ini`) and cover image (`kfmon-kobo_2048.png`)
already ship in `kobo_2048.zip` — only the KFMon prerequisite is extra:
...
2. After installing the app, a book called **kfmon-kobo_2048** appears in
...
- `.adds/kobo_2048/` (the app, its settings and saved progress)
- `.adds/nm/kobo_2048` (the NickelMenu entry)
- `.adds/kfmon/config/kobo_2048.ini` and `kfmon-kobo_2048.png` (the KFMon launcher)
