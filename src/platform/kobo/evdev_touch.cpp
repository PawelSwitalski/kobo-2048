...
bool EvdevTouch::init(const DisplayInfo& display) {
    ...
    swapXY_ = envFlag("KOBO_2048_TOUCH_SWAP_XY");
    mirrorX_ = envFlag("KOBO_2048_TOUCH_MIRROR_X");
    mirrorY_ = envFlag("KOBO_2048_TOUCH_MIRROR_Y");
    debug_ = envFlag("KOBO_2048_TOUCH_DEBUG");
...
void EvdevTouch::mapRaw(int rawX, int rawY, int& outX, int& outY) const {
    int x = rawX, y = rawY;
    ...
    int px = (maxX > minX)
                 ? static_cast<int>(static_cast<long long>(x - minX) * (viewW_ - 1) / (maxX - minX))
                 : x;
    int py = (maxY > minY)
                 ? static_cast<int>(static_cast<long long>(y - minY) * (viewH_ - 1) / (maxY - minY))
                 : y;
    ...
    while (true) {
        ...
        for (size_t k = 0; k < static_cast<size_t>(n) / sizeof(input_event); ++k) {
            ...
            if (e.type == EV_ABS) {
                ...
                // First complete raw position seen this gesture is the touch-down point.
                if (downRawX < 0 && rawX >= 0 && rawY >= 0) {
                    downRawX = rawX;
                    downRawY = rawY;
                }
                if (lifted && sawContact && rawX >= 0 && rawY >= 0) {
                    int px, py, spx, spy;
                    mapRaw(rawX, rawY, px, py);
                    mapRaw(downRawX >= 0 ? downRawX : rawX, downRawY >= 0 ? downRawY : rawY, spx,
                           spy);
                    if (debug_)
                        std::fprintf(stderr, "tap raw=(%d,%d) -> (%d,%d) start=(%d,%d)\n", rawX,
                                     rawY, px, py, spx, spy);
                    return Tap{px, py, spx, spy};
                }
                lifted = false;
            }
        }
